...
configure_{{ id }}:
 cmd.run:
  - name: "/usr/local/bin/wp core config --dbname=\"{{ site.database }}\" --dbuser=\"{{ salt['config.get']('site.dbuser') }}\" --dbpass=\"{{ salt['config.get']('site.dbpass') }}\" --dbhost=\"{{ site.dbhost }}\" --path=\"{{ map.docroot }}/{{ id }}\""
  - cwd: {{ map.docroot }}/{{ id }}
  - user: {{ map.www_user }}
  - unless: test -f {{ map.docroot }}/{{ id }}/wp-config.php  
...
install_{{ id }}:
 cmd.run:
  - cwd: {{ map.docroot }}/{{ id }}
  - name: "/usr/local/bin/wp core install --url=\"{{ site.url }}\" --title=\"{{ site.title }}\" --admin_user=\"{{ salt['config.get']('site.username') }}\" --admin_password=\"{{ salt['config.get']('site.password') }}\" --admin_email=\"{{ site.email }}\" --path=\"{{ map.docroot }}/{{ id }}/\""
  - user: {{ map.www_user }}
